-- =====================================================================
--  AKCIJE — FAZA 1: NACIONALNE PONUDE ZA SVIH ~8.200 PLZ-ova
-- =====================================================================
--  Problem: pokrivali smo samo 6 gradova. Ko unese bilo koji drugi PLZ,
--  ne vidi ništa — iako Kaufland i Lidl imaju ISTE cijene u cijeloj
--  Njemačkoj, pa nema razloga da ih vežemo za grad.
--
--  Rješenje: kolona `scope`.
--     'DE'                     → vrijedi SVUGDJE (Kaufland, Lidl…)
--     'aldi-sued'/'aldi-nord'  → regija lanca
--     NULL                     → stari redovi; ponašaju se kao i do sada
--                                (vide se samo u svom PLZ-u)
--
--  Nacionalni redovi se u bazi drže pod PLZ-om '00000' — to nije grad,
--  nego "kanta". Upit ih dodaje SVAKOM korisniku, koji god PLZ upisao.
--
--  Aldi za sada ostaje na 6 gradova — njegova granica (Nord/Süd) dolazi
--  u fazi 2, kad napravimo mapu PLZ→regija iz Aldijevih spiskova filijala.
--
--  Sadrži SVE iz `akcije-uvoz.sql` (datumi + ručni uvoz) — ako ideš iznova,
--  dovoljno je pokrenuti OVAJ fajl. Bezopasno više puta.
-- =====================================================================

alter table ak_discounts
    add column if not exists scope text;

comment on column ak_discounts.scope is
    'DE = vrijedi u cijeloj Njemačkoj | aldi-sued/aldi-nord = regija lanca | NULL = stari red, samo svoj PLZ';

create index if not exists ak_discounts_scope_idx on ak_discounts (scope, date);
...
begin
    ...
    return query execute format($q$
        -- Najsvježiji dan gleda i nacionalne i lokalne redove (scraper ih
        -- upisuje u istom prolazu, pa je datum isti).
        with snap as (
            select max(date) as date
              from ak_discounts
             where source <> 'manual'
               and (scope = 'DE' or plz = $1)
        )
        select
        ...
        cross join snap
        where not d.hidden
          -- NOVO: nacionalne ponude vrijede za SVAKI PLZ
          and (d.scope = 'DE' or d.plz = $1)
...
-- ---------------------------------------------------------------------
-- 2) Prodavnice — izvode se IZ PONUDA, ne iz ak_stores_by_plz.
--    Nacionalni lanci nisu vezani ni za jedan grad, pa ih stara veza
--    ne bi ni našla. Ovako je lista uvijek tačna: prikaže se ono što
--    korisnik stvarno može otvoriti.
-- ---------------------------------------------------------------------
...
as $fn$
    with snap as (
        select max(date) as date
          from ak_discounts
         where source <> 'manual' and (scope = 'DE' or plz = p_plz)
    )
