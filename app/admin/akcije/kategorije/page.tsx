...
export default function AkcijeKategorije() {
  const [items, setItems] = useState<Row[] | null>(null);
  const [datum, setDatum] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/admin/akcije/liste?what=kategorije", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => {
        setItems(j.items || []);
        setDatum(j.datum);
  ...
  return (
    <div>
      <h1 style={{ fontSize: 26, fontWeight: 800, margin: 0 }}>Kategorije</h1>
      <div style={{ color: "#6b7280", fontSize: 13.5, margin: "4px 0 18px" }}>
        Kategorije iz današnjeg snapshota · {datum || "danas"}
      </div>
      ...
      <div style={panel}>
        {!items ? (
          ...
          <div style={empty}>Još nema kategorija.</div>
        ) : (
          <table style={table}>
            <thead>
              <tr>
            ...
            <tbody>
              {items.map((r) => (
                <tr key={r.kategorija}>
                  <td style={{ ...td, fontWeight: 600 }}>{r.kategorija}</td>
                  <td style={{ ...td, textAlign: "right" }}>{r.broj}</td>
                  <td style={{ ...td, textAlign: "right", color: "#6b7280" }}>{r.saSlikom}</td>
                </tr>
              ))}
            </tbody>
          </table>
